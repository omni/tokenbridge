...
const { COMMON_HOME_RPC_URL, COMMON_FOREIGN_RPC_URL, COMMON_RPC_REQUEST_TIMEOUT } = process.env
...
const options = {
  requestTimeout: parseInt(COMMON_RPC_REQUEST_TIMEOUT, 10) || 0,
  retry: RETRY_CONFIG
}

const homeProvider = new HttpListProvider(homeUrls, options)
const web3Home = new Web3(homeProvider)

const foreignProvider = new HttpListProvider(foreignUrls, options)
const web3Foreign = new Web3(foreignProvider)

const redundantHomeProvider = new RedundantHttpListProvider(homeUrls, options)
const web3HomeRedundant = new Web3(redundantHomeProvider)

const redundantForeignProvider = new RedundantHttpListProvider(foreignUrls, options)
